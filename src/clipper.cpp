...
TEdge* ClipperBase::ProcessBound(TEdge* E, bool NextIsForward)
{
    ...
    if (NextIsForward) {
        ...
        if (IsHorizontal(*Result) && Result->Next->OutIdx != Skip) {
            ...
            // vertex unless a Skip edge is encountered when that becomes the top
            // divide
            Horz = Result;
        ...
        if (E->Prev == E->Next)
            break;  // only two vertices
                 ...
                 SlopesEqual(E->Prev->Curr, E->Curr, E->Next->Curr,
                             m_UseFullRange) &&
                 (!m_PreserveCollinear ||
                  !Pt2IsBetweenPt1AndPt3(E->Prev->Curr, E->Curr,
                                         E->Next->Curr))) {
            // Collinear edges are allowed for open paths but in closed paths
            // the default is to merge adjacent collinear edges into a single
            // edge. However, if the PreserveCollinear property is enabled, only
            // overlapping collinear edges (ie spikes) will be removed from
            // closed paths.
            if (E == eStart)
                eStart = E->Next;
            E = RemoveEdge(E);
    ...
    while (PopLocalMinima(botY, lm)) {
        ...
        if (Op1 && IsHorizontal(*rb) && m_GhostJoins.size() > 0 &&
            ...
            for (JoinList::size_type i = 0; i < m_GhostJoins.size(); ++i) {
                ...
                // convert the 'ghost' join to a real join ready for later ...
        ...
        if (lb->OutIdx >= 0 && lb->PrevInAEL &&
            ...
            SlopesEqual(lb->PrevInAEL->Bot, lb->PrevInAEL->Top, lb->Curr,
                        lb->Top, m_UseFullRange) &&
        ...
        if (lb->NextInAEL != rb) {
            ...
            if (e) {
                while (e != rb) {
                    ...
                    // assumes that param1 will be to the Right of param2 ABOVE
                    // the intersection ...
...
void Clipper::ProcessHorizontal(TEdge* horzEdge)
{
    ...
    for (;;)  // loop through consec. horizontal edges
    {
        ...
        while (e) {
            ...
            // output polygons) whereever maxima touch these horizontal edges.
            // This helps 'simplifying' polygons (ie if the Simplify property is
            //set).
            if (m_Maxima.size() > 0) {
                if (dir == dLeftToRight) {
            ...
            // edge ... nb: Smaller Dx's are to the right of larger Dx's ABOVE
            // the horizontal.
...
bool Clipper::FixupIntersectionOrder()
{
    ...
    CopyAELToSEL();
    std::sort(m_IntersectList.begin(), m_IntersectList.end(),
              IntersectListSort);
    ...
    for (size_t i = 0; i < cnt; ++i) {
        ...
        SwapPositionsInSEL(m_IntersectList[i]->Edge1,
...
void Clipper::ProcessEdgesAtTopOfScanbeam(const cInt topY)
{
    ...
    while (e) {
        ...
        if (IsMaximaEdge) {
            ...
            // then make sure both edges have a vertex here ...
            if (m_StrictSimple) {
                TEdge* ePrev = e->PrevInAEL;
...
bool Clipper::JoinPoints(Join* j, OutRec* outRec1, OutRec* outRec2)
{
    ...
    if (isHorizontal && (j->OffPt == j->OutPt1->Pt) &&
        ...
        // OutPt2.Pt may be anywhere along the horizontal edge.
        op1b = op1;
        ...
        // edges
        if (!GetOverlap(op1->Pt.X, op1b->Pt.X, op2->Pt.X, op2b->Pt.X, Left,
                        Right))
            return false;

        // DiscardLeftSide: when overlapping edges are joined, a spike will
        // created which needs to be cleaned up. However, we don't want Op1 or
        // Op2 caught up on the discard Side as either may still be needed for
        // other joins ...
        IntPoint Pt;
...
void Clipper::FixupFirstLefts2(OutRec* InnerOutRec, OutRec* OuterOutRec)
{
    ...
    // check every polygon that's also contained by OuterOutRec's FirstLeft
    // container (including 0) to see if they've become inner to the new inner
    //polygon ...
            ...
            if (node.m_jointype == jtRound) {
                ...
                for (cInt j = 1; j <= steps; j++) {
                    m_destPoly.push_back(
                        IntPoint(Round(m_srcPoly[0].X + X * delta),
...
{
    double dx =
        std::tan(std::atan2(m_sinA, m_normals[k].X * m_normals[j].X +
                                        m_normals[k].Y * m_normals[j].Y) /
                 4);
    m_destPoly.push_back(
        IntPoint(Round(m_srcPoly[j].X +
