...
int TRTDBnet::init(const std::string& ini_path)
{
    ...
    m_input_size = reader.GetInteger("tensorrt", "input_size", 640);
    ...
    m_expand_ratio = reader.GetReal("tensorrt", "expand_ratio", 1.5);
    return 0;
}
...
int TRTDBnet::load_model(std::string& onnx_file, std::string& engine_file)
{
    Logger_dbnet gLogger;
    cudaSetDevice(m_gpu_index);
    std::ifstream intrt(engine_file, std::ios::binary);
...
// // Creat the engine using only the API and not any parser.backbone is
// resnet18 ICudaEngine* TRTDBnet::createEngine(unsigned int m_max_batchsize,
//                                     IBuilder* builder, IBuilderConfig*
//                                     config, DataType dt, std::string&
//                                     wts_file)
// {
...
//         *data, 64, DimsHW{7, 7}, weightMap["backbone.conv1.weight"],
//         emptywts);
//     assert(conv1);
//     conv1->setStride(DimsHW{2, 2});
//     conv1->setPadding(DimsHW{3, 3});

//     IScaleLayer* bn1 = addBatchNorm2d(network, weightMap,
//     *conv1->getOutput(0),
//                                       "backbone.bn1", 1e-5);
...
//     /* ------- FPN  neck ------- */
//     ILayer* p5 = convBnLeaky(network, weightMap, *relu9->getOutput(0), 64, 1,
//     1,
//                              1, "neck.reduce_conv_c5.conv",
//                              ".bn");  // k=1 s = 1  p = k/2=1/2=0
//     ILayer* c4_1 = convBnLeaky(network, weightMap, *relu7->getOutput(0), 64,
//     1,
//                                1, 1, "neck.reduce_conv_c4.conv", ".bn");

//     float* deval = reinterpret_cast<float*>(malloc(sizeof(float) * 64 * 2 *
//     2)); for (int i = 0; i < 64 * 2 * 2; i++) {
//         deval[i] = 1.0;
//     }
...
//         *p4_1->getOutput(0), *c4_1->getOutput(0),
//         ElementWiseOperation::kSUM);
//     ILayer* p4 = convBnLeaky(network, weightMap, *p4_add->getOutput(0), 64,
//     3,
//                              1, 1, "neck.smooth_p4.conv", ".bn");  // smooth
//     ILayer* c3_1 = convBnLeaky(network, weightMap, *relu5->getOutput(0), 64,
//     1,
...
//         *p3_1->getOutput(0), *c3_1->getOutput(0),
//         ElementWiseOperation::kSUM);
//     ILayer* p3 = convBnLeaky(network, weightMap, *p3_add->getOutput(0), 64,
//     3,
//                              1, 1, "neck.smooth_p3.conv", ".bn");  // smooth
//     ILayer* c2_1 = convBnLeaky(network, weightMap, *relu3->getOutput(0), 64,
//     1,
...
//         *p2_1->getOutput(0), *c2_1->getOutput(0),
//         ElementWiseOperation::kSUM);
//     ILayer* p2 = convBnLeaky(network, weightMap, *p2_add->getOutput(0), 64,
//     3,
...
//                                p4_up_p2->getOutput(0),
//                                p5_up_p2->getOutput(0)};
//     IConcatenationLayer* neck_cat = network->addConcatenation(inputTensors,
//     4);

//     ILayer* neck_out =
//         convBnLeaky(network, weightMap, *neck_cat->getOutput(0), 256, 3, 1,
//         1,
//                     "neck.conv.0", ".1");  // smooth
//     assert(neck_out);
//     ILayer* binarize1 = convBnLeaky(network, weightMap,
//     *neck_out->getOutput(0),
...
//         network, weightMap, *binarizeup->getOutput(0), "head.binarize.4",
//         1e-5);
...
//         *binarizerelu1->getOutput(0), 64, DimsHW{2, 2}, deconvwts8,
//         emptywts);
...
//         weightMap["head.binarize.7.weight"],
//         weightMap["head.binarize.7.bias"]);
//     assert(binarize3);
...
//         weightMap["head.thresh.3.1.weight"],
//         weightMap["head.thresh.3.1.bias"]);
//     assert(thresh2);
...
//         network->addActivation(*threshbn1->getOutput(0),
...
//         weightMap["head.thresh.6.1.weight"],
//         weightMap["head.thresh.6.1.bias"]);
//     assert(thresh3);
...
//     ITensor* inputTensors2[] = {binarize4->getOutput(0),
//     thresh4->getOutput(0)}; IConcatenationLayer* head_out =
//     network->addConcatenation(inputTensors2, 2);
...
std::vector<TRTDBnetResult> TRTDBnet::detect(cv::Mat& image_ori)
{
    ...
    if (image.cols > 1920 || image.rows < 32) {
        return results;
    }
    ...
    // for (int i = 0; i < 100;i++){
    //     std::cout << input[image.rows*image.cols*2+300*image.cols+300+i] << "
    //     ";
    //     //std::cout << input[i] << " ";
    // }
    // std::cout<<std::endl;
    ...
    // for (int i = 0; i < 100;i++){
    //     //std::cout << output[image.rows*image.cols+10*image.cols+i] << " ";
    //     //std::cout << output[i] << " ";
    //     std::cout << output[300*image.cols+300+i] << " ";
    // }
    // std::cout<<std::endl;
    ...
    // cv::imwrite("./bit_map.jpg", map);
    // 提取最小外接矩形
    std::vector<std::vector<cv::Point>> contours;
    // std::vector<cv::Vec4i> hierarcy;
    cv::findContours(map, contours, cv::RETR_LIST, cv::CHAIN_APPROX_SIMPLE);
    ...
    for (int i = 0; i < choose_box; i++) {
        ...
        for (int j = 0; j < 4; j++) {
            ...
            one_result.box_coordinates[j * 2] =
                std::round(float(rect[j].x) / image.cols * ori_w);
            // floor(rect[j].x/scale);
            one_result.box_coordinates[j * 2 + 1] =
                std::round(float(rect[j].y) / image.rows * ori_h);
        ...
        std::sort(results.begin(), results.end(),
                  [](TRTDBnetResult& a, TRTDBnetResult& b) {
                      return a.box_coordinates[1] < b.box_coordinates[1];
                  });
        for (int i = 0; i < results.size() - 1; i++) {
            if ((results[i].box_coordinates[1] +
                 results[i].box_coordinates[7]) /
                    2 >
                results[i + 1].box_coordinates[1]) {
                if (results[i].box_coordinates[0] >
                    results[i + 1].box_coordinates[0]) {
                    auto change = results[i];
