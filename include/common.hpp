                    ...
                    std::string bnname, bool bias = true)
{
    ...
    // *conv1->getOutput(0), lname + ".bn", 1e-4);
    IScaleLayer* bn1 =
        addBatchNorm2d(network, weightMap, *conv1->getOutput(0),
                       lname.substr(0, lname.find_last_of(".")) + bnname, 1e-3);
    auto lr =
...
IActivationLayer* basicBlock(INetworkDefinition* network,
                             std::map<std::string, Weights>& weightMap,
                             ...
                             std::string lname)
{
    ...
    IConvolutionLayer* conv1 =
        network->addConvolution(input, outch, DimsHW{3, 3},
    ...
    IConvolutionLayer* conv2 =
        network->addConvolution(*relu1->getOutput(0), outch, DimsHW{3, 3},
    ...
    if (inch != outch) {
        IConvolutionLayer* conv3 = network->addConvolution(
            input, outch, DimsHW{1, 1},
            weightMap[lname + "downsample.0.weight"], emptywts);
        assert(conv3);
        conv3->setStride(DimsHW{stride, stride});
        IScaleLayer* bn3 =
            addBatchNorm2d(network, weightMap, *conv3->getOutput(0),
                           lname + "downsample.1", 1e-5);
        ew1 = network->addElementWise(*bn3->getOutput(0), *bn2->getOutput(0),
